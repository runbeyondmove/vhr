      ...
      <el-main style="padding-left: 0px;padding-top: 0px">
        <div>
          <el-table
            ...
            <el-table-column
              label="调薪日期">
              <template slot-scope="scope">{{ scope.row.asDate | dateformat('YYYY-MM-DD HH:mm:ss')}}</template>
            </el-table-column>
            <el-table-column
              width="200"
              prop="beforesalary"
    ...
    methods: {
      ...
      loadAdjustsalarysCfg(){
        ...
        this.getRequest("/personnel/adjustsalary?keywords="+_this.keywords+"&page=" + this.currentPage).then(resp=> {
          ...
          console.log(resp);
          // if (resp && resp.status == 200) {
            _this.adjustsalarys = resp.adjustsalaryList;
            _this.totalCount = resp.count;
          // }
